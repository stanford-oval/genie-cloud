...
const MicInputButton = (props) => {
  ...
  const postAudio = (blob) => {
    ...
    axios({
      ...
      if (response.data.success)
        props.setCommand(response.data.command); // Update command
      else
  ...
  const startStopRecord = () => {
    if (!isRecording) {
      // Start recording
      navigator.mediaDevices.getUserMedia({ audio: true, video: false }).then((strm) => {
        console.log('getUserMedia() success, stream created, initializing Recorder.js ...');
        const audioContext = new (window.AudioContext || window.webkitAudioContext)();
        const rec = new Recorder(audioContext.createMediaStreamSource(strm), { numChannels: 1 });
        rec.record();
  ...
  return (
    <div id='micInputButton'>
      <FontAwesomeIcon
        ...
        size='2x' />
      <Button variant='primary' onClick={ startStopRecord }>{isRecording ? 'Stop Recording' : 'Record'}</Button>
    </div>
  );
};
