...
function computeDistanceFromRoot(grammar, minDistanceFromRoot) {
    let queue = [];
...
function preprocessGrammar(grammar, averagePruningFactor) {
    for (let category in grammar) {
        let preprocessed = [];
        let prunefactors = [];
...
function *generate(grammar, options) {
    ...
    preprocessGrammar(grammar, averagePruningFactor);
    const minDistanceFromRoot = {};
    computeDistanceFromRoot(grammar, minDistanceFromRoot);
