...
everything.get('/devices', iv.validateGET({
    ...commonQueryArgs,
    class: '?string'
}), (req, res, next) => {
...
**/
v3.get('/schema/:schemas', iv.validateGET(commonQueryArgs), (req, res, next) => {
    ...
    client.getSchemas(schemas, withMetadata).then((obj) => {
        res.set('Vary', 'Accept');
 ...
 */
v3.get('/devices/code/:kind', iv.validateGET(commonQueryArgs), (req, res, next) => {
    const accept = accepts(req).types(['application/x-thingtalk', 'text/html']);
    if (!accept) {
        res.status(405).json({ error: 'must accept application/x-thingtalk' });
        return;
 ...
 *  }
 */
v3.get('/devices/setup/:kinds', iv.validateGET(commonQueryArgs), (req, res, next) => {
    const kinds = req.params.kinds.split(',');
    const client = new ThingpediaClient(req.query.developer_key, req.query.locale, req.query.thingtalk_version);
 ...
 */
v3.get('/devices/package/:kind', iv.validateGET({
    ...commonQueryArgs,
    version: '?integer'
}), (req, res, next) => {
    const kind = req.params.kind;
    const client = new ThingpediaClient(req.query.developer_key, req.query.locale, req.query.thingtalk_version);
    client.getModuleLocation(kind, req.query.version ? Number(req.query.version) : undefined).then((location) => {
        res.cacheFor(60000);
        res.redirect(302, location);
    }).catch(next);
});

function isValidDeviceClass(req : express.Request, res : express.Response) {
    if (req.query.class &&
        ['online', 'physical', 'data', 'system',
         'media', 'social-network', 'home', 'communication',
         'health', 'service', 'data-management'].indexOf(req.query.class as string) < 0) {
 ...
 *  }
 */
v3.get('/devices/setup', iv.validateGET({
    ...commonQueryArgs,
    class: '?string'
}), (req, res, next) => {
    if (!isValidDeviceClass(req, res))
...
v3.get('/devices/all', iv.validateGET({
    ...
    const client = new ThingpediaClient(req.query.developer_key, req.query.locale, req.query.thingtalk_version);
    client.getDeviceList(req.query.class, page, page_size).then((devices) => {
        res.cacheFor(86400000);
 ...
 *  }
 */
v3.get('/devices/search', iv.validateGET({
    ...commonQueryArgs,
    q: 'string'
}), (req, res, next) => {
    const q = req.query.q;
    const client = new ThingpediaClient(req.query.developer_key, req.query.locale, req.query.thingtalk_version);
    client.searchDevice(q).then((devices) => {
        res.cacheFor(86400000);
        res.json({ result: 'ok', data: devices });
    }).catch(next);
});
 ...
 *  }
 */
v3.get('/commands/search', iv.validateGET({
    ...commonQueryArgs,
    q: 'string'
}), (req, res, next) => {
    const q = req.query.q;
...
v3.post('/devices/discovery', iv.validateGET(commonQueryArgs), (req, res, next) => {
    const client = new ThingpediaClient(req.query.developer_key, req.query.locale, req.query.thingtalk_version);

    client.getKindByDiscovery(req.body).then((result) => {
        res.cacheFor(86400000);
        res.status(200).json({ result: 'ok', data: { kind: result } });
    }).catch(next);
});
 ...
 *
 */
v3.get('/examples/by-kinds/:kinds', iv.validateGET(commonQueryArgs), (req, res, next) => {
    const kinds = req.params.kinds.split(',');
    const client = new ThingpediaClient(req.query.developer_key, req.query.locale, req.query.thingtalk_version);
    ...
    client.getExamplesByKinds(kinds, accept as string).then((result) => {
        res.set('Vary', 'Accept');
        res.cacheFor(300000);
        res.status(200);
        res.set('Content-Type', accept === 'text/html' ? 'text/plain' : accept);
 ...
 *
 */
v3.get('/examples/all', iv.validateGET(commonQueryArgs), (req, res, next) => {
    const client = new ThingpediaClient(req.query.developer_key, req.query.locale, req.query.thingtalk_version);
    const accept = accepts(req).types(['application/x-thingtalk', 'application/x-thingtalk;editMode=1', 'text/html']);
    if (!accept) {
        res.status(405).json({ error: 'must accept application/x-thingtalk' });
    ...
    client.getAllExamples(accept as string).then((result) => {
        res.set('Vary', 'Accept');
        res.cacheFor(300000);
        res.status(200);
        res.set('Content-Type', accept === 'text/html' ? 'text/plain' : accept);
 ...
 *
 */
v3.get('/examples/search', iv.validateGET({
    ...commonQueryArgs,
    q: 'string'
}), (req, res, next) => {
    const accept = accepts(req).types(['application/x-thingtalk', 'application/x-thingtalk;editMode=1', 'text/html']);
    ...
    const client = new ThingpediaClient(req.query.developer_key, req.query.locale, req.query.thingtalk_version);
    client.getExamplesByKey(req.query.q, accept as string).then((result) => {
        res.cacheFor(300000);
        res.status(200);
        res.set('Content-Type', accept === 'text/html' ? 'text/plain' : accept);
        if (typeof result === 'string')
 ...
 *
 */
v3.post('/examples/click/:id', iv.validateGET(commonQueryArgs), (req, res, next) => {
    const client = new ThingpediaClient(req.query.developer_key, req.query.locale, req.query.thingtalk_version);

    client.clickExample(Number(req.params.id)).then(() => {
        res.cacheFor(300000);
        res.status(200).json({ result: 'ok' });
    }).catch(next);
});

function getAllEntities(req : express.Request<any, any, any, { snapshot ?: string, developer_key ?: string, locale ?: string, thingtalk_version ?: string }>, res : express.Response, next : express.NextFunction) {
    const snapshotId = req.query.snapshot ? parseInt(req.query.snapshot) : -1;
 ...
 *
 */
v3.get('/entities/all', iv.validateGET({
    ...commonQueryArgs,
    snapshot: '?string'
}), getAllEntities);
 ...
 *
 */
v3.get('/entities/lookup', iv.validateGET({
    ...commonQueryArgs,
    q: 'string'
}), (req, res, next) => {
 ...
 *
 */
v3.get('/entities/lookup/:type', iv.validateGET({
    ...commonQueryArgs,
    q: 'string'
}), (req, res, next) => {
    const language = (req.query.locale || 'en').split(/[-_@.]/)[0];
...
function getAllStrings(req : express.Request<any, any, any, { developer_key ?: string, locale ?: string, thingtalk_version ?: string }>, res : express.Response, next : express.NextFunction) {
    const client = new ThingpediaClient(req.query.developer_key, req.query.locale, req.query.thingtalk_version);
 ...
 *
 */
v3.get('/strings/all', iv.validateGET(commonQueryArgs), getAllStrings);
 ...
 *
 */
v3.get('/strings/list/:type', iv.validateGET({
    ...commonQueryArgs,
    developer_key: 'string'
}), (req, res, next) => {
 ...
 *
 */
v3.get('/locations/lookup', iv.validateGET({
    ...commonQueryArgs,
    q: 'string',
    latitude: '?number',
    longitude: '?number'
    ...
    client.lookupLocation(searchKey, req.query.latitude && req.query.longitude ? { latitude: Number(req.query.latitude), longitude: Number(req.query.longitude) } : undefined).then((data) => {
        res.cacheFor(300000);
        res.status(200).json({ result: 'ok', data });
    }).catch(next);
});
...
function getSnapshot(req : express.Request<any, any, any, {
    locale ?: string,
    developer_key ?: string;
    thingtalk_version ?: string;
    meta ?: string;
}>, res : express.Response, next : express.NextFunction, accept : string) {
...
v3.get('/snapshot/:id', iv.validateGET({
    ...commonQueryArgs,
    meta: '?string'
}), (req, res, next) => {
    const accept = accepts(req).types(['application/x-thingtalk', 'text/html']);
    ...
    getSnapshot(req, res, next, accept as string);
});
