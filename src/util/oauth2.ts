...
function authOAuth2Client(clientId : unknown, clientSecret : unknown, done : (err ?: Error|null, client ?: model.Row|false) => void) {
...
server.grant(oauth2orize.grant.code((client, redirectURI, user, ares, done) => {
    jwt.sign({
        ...
        scope: ares.scope
    } as AuthorizationCode, secret.getJWTSigningKey(), { expiresIn: 600 /* seconds */ }, done);
}));
...
server.exchange(oauth2orize.exchange.code((client, code, redirectURI, done) => {
    jwt.verify(code, secret.getJWTSigningKey(), {
        ...
        const decoded = decodedobj as AuthorizationCode|undefined;
        if (err || !decoded || decoded.grant_type !== 'authorization_code' ||
        ...
        db.withTransaction(async (dbClient) => {
            ...
            const refreshToken = await util.promisify<object, string, jwt.SignOptions, string>(jwt.sign)({
                sub: client.id,
                user_id: decoded.user_id,
                grant_type: 'refresh_token',
                scope: decoded.scope
            } as RefreshToken, secret.getJWTSigningKey(), { /* never expires */ });
            ...
            for (const scope of decoded.scope)
                allScopes.add(scope);
            ...
            const accessToken = await util.promisify<object, string, jwt.SignOptions, string>(jwt.sign)({
                sub: decoded.user_id,
                aud: 'oauth2',
                scope: decoded.scope
...
server.exchange(oauth2orize.exchange.refreshToken((client, refreshToken, scope, done) => {
    db.withClient(async (dbClient) => {
        ...
        try {
            decoded = await util.promisify<string, string, jwt.VerifyOptions, any>(jwt.verify)(refreshToken, secret.getJWTSigningKey(), {
                algorithms: ['HS256'],
                subject: client.id,
                clockTolerance: 30,
            });
