...
things, while hiding the details of configuration and communication of the devices.
...
A ThingTalk program is composed of a list of _rules_. 
...
and one can put multiple rules in a pair of curly brackets to make a more complicated ThingTalk program. 
...
when the program started. The query invokes the `get` function from device 
...
Each query can be _monitored_ (unless marked as not monitorable by the developer) to become a stream,
...
A short-hand syntax for timer with interval of one day is provided:
```tt
...
Note that if the rule also contains monitor stream,
both the data from the stream and query will be available to use for the action. 

## Action - what does the program do at the end? 
After a rule gets triggered, data gets retrieved, we finally run action. 
`notify` is the default action which shows the data from stream and query to the user
in Almond. 
Besides `notify`, the action part can also invoke any action function in Thingpedia. 
For example, instead of getting notified inside Almond, you can choose to send the information
to slack as follows:
```tt
...
### Parameter passing
In addition to constant values, we can also pass the return value from functions in front.
```tt
monitor @com.foxnews.get() => @com.slack.send(channel="general", message=title);
```

### Filtering on output parameters
What if the user does not want to get notified by all news, but only those related to specific topics? 
...
In this example, we filter on `title` parameter, and only if the title contains (denoted by `=~`)
the word "Stanford", the rule will get triggered. 
...
Code snippets are used to provide composable examples for Thingpedia Devices,
...
on Fox News, and only run the Bing search once it detects something new. 
